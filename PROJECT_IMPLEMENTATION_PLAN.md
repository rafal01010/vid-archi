...
- The Svelte player now makes `selectedQuality` an explicit dependency of the source-sync path so the dropdown change immediately triggers the player rebuild instead of relying on an inferred reactive dependency through a helper function call.
- The homepage library now treats opening a video as the primary action: the card body is the clickable surface, the `Open video` CTA is visually dominant, status text is quieter, and delete affordances stay available but visually secondary.
- The share-link box now uses a clipboard fallback path for non-secure or restricted browser contexts so `Copy link` still works when `navigator.clipboard.writeText()` is unavailable.
- Portrait videos are now treated as first-class uploads in both playback and processing: the frontend player renders portrait media with `object-fit: contain` inside the existing player frame, and chunker/transcoder rendition eligibility compares normalized long-edge/short-edge bounds so vertical `2160x3840` sources can still qualify for higher portrait renditions such as `1080p` and `2160p`.
- The chunker source probe now reads ffprobe rotation metadata (`side_data.rotation` or `tags.rotate`) and normalizes stored source dimensions before dispatching transcoding jobs. This is required for phone-shot portrait uploads whose files are encoded as landscape pixels plus a display-rotation flag.
- The homepage recent-videos view is now a denser vertical list rather than a multi-column card grid, closer to a video-feed scan pattern. Shared frontend spacing, panel padding, controls, and headings are also scaled down so more of each page fits without excessive scrolling.
- The homepage list rows are now cleaner: the primary CTA is a left-aligned `Open` button, repeated helper descriptions are removed, the list-row delete affordance is smaller, and primary buttons use flat dark fills instead of gradients.
